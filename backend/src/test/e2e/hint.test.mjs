...
describe('E2e tests hint', () => {
  describe('POST /api/hint/add_hint', () => {
    ...
    it('should return 401 if no token is provided', async () => {
      const res = await chai.request.execute(app).post('/api/hint/add_hint').send(hint().build());
    ...
    it('should return 400 if action is missing', async () => {
      ...
      expect(res.body).to.be.deep.equal({
        errors: ['Invalid value', 'action is required', 'Invalid value for action'],
      });
    });
    ...
    it('should return 400 if headerColor is invalid', async () => {
      ...
      expect(res).to.have.status(400);
      expect(res.body).to.be.deep.equal({
        errors: ['Invalid value for headerColor'],
      });
    });
    it('should return 400 if textColor is invalid', async () => {
      ...
      expect(res).to.have.status(400);
      expect(res.body).to.be.deep.equal({
        errors: ['Invalid value for textColor'],
      });
    });
    it('should return 400 if buttonBackgroundColor is invalid', async () => {
      ...
      expect(res).to.have.status(400);
      expect(res.body).to.be.deep.equal({
        errors: ['Invalid value for buttonBackgroundColor'],
      });
    });
    it('should return 400 if buttonTextColor is invalid', async () => {
      ...
      expect(res).to.have.status(400);
      expect(res.body).to.be.deep.equal({
        errors: ['Invalid value for buttonTextColor'],
      });
    });
  ...
  describe('DELETE /api/hint/delete_hint/:hintId', () => {
    ...
    it('should return 401 if no token is provided', async () => {
      const res = await chai.request.execute(app).delete('/api/hint/delete_hint/1').send();
      expect(res).to.have.status(401);
      expect(res.body).to.be.deep.equal({ error: 'No token provided' });
    });
    it('should return 403 if user does not have required access level', async () => {
  ...
  describe('PUT /api/hint/edit_hint/:hintId', () => {
    ...
    it('should return 401 if no token is provided', async () => {
      const res = await chai.request.execute(app).put('/api/hint/edit_hint/1').send(hint().build());
      expect(res).to.have.status(401);
      expect(res.body).to.be.deep.equal({ error: 'No token provided' });
    });
    it('should return 403 if user does not have required access level', async () => {
  ...
  describe('GET /api/hint/all_hints', () => {
      ...
      await Promise.all(
        hintList.map((popup) => {
          return chai.request
            .execute(app)
            .post('/api/hint/add_hint')
            .set('Authorization', `Bearer ${popup.createdBy === 1 ? token : newToken}`)
            .send(popup);
        })
      );
    });
    ...
    it('should return 200 if all hints are returned', async () => {
      const res = await chai.request.execute(app).get('/api/hint/all_hints').set('Authorization', `Bearer ${token}`);
      expect(res).to.have.status(200);
  ...
  describe('GET /api/hint/hints', () => {
    ...
    it('should return 200 if hints are returned', async () => {
      const res = await chai.request.execute(app).get('/api/hint/hints').set('Authorization', `Bearer ${token}`);
      expect(res).to.have.status(200);
  ...
  describe('GET /api/hint/get_hint/:id', () => {
    ...
    it('should return 404 if hint is not found', async () => {
      const res = await chai.request.execute(app).get('/api/hint/get_hint/1').set('Authorization', `Bearer ${token}`);
      expect(res).to.have.status(404);
      expect(res.body).to.be.deep.equal({
        errors: [{ msg: 'Hint not found' }],
      });
